...
#
# Planner (Planner class): Uses MPPI/CEM style sampling with mixture of policy prior
...
class TDMPC2(torch.nn.Module):
	...
	def __init__(self, cfg):
		...
		# planner: wraps MPPI/CEM logic; caches mean/std tensors shaped (T, A)
		self.planner = Planner(cfg, self.model, self.scale, self.discount, self.device)
		if cfg.compile:
			...
			@torch.compile(mode=self.cfg.compile_type, fullgraph=False, dynamic=True)
			def optim_step():
				self.optim.step()
				return
	...
	def reset_planner_state(self):
		if hasattr(self, 'planner'):
			# reset cached means/stds -> tensors (T, A) and (T, parents, A)
		...
		with maybe_range('Agent/act', self.cfg):
			if task is not None:
				if isinstance(task, torch.Tensor):
					# task: (1,) long tensor for multitask setups
					task = task.to(self.device)
				else:
					# wrap scalar idx -> (1,) tensor on device
					task = torch.tensor([task], device=self.device)
			log_detailed = bool(getattr(self, 'log_detailed', False))  # flag toggles planner trace capture
			if mpc:
				if self.cfg.enable_train_planner and not eval_mode:
					# plan_result.distribution.actions: (T, parents, A)
					plan_result = self.planner.plan_train(obs, task=task, log_detailed=log_detailed)
				else:
					plan_result = self.planner.plan_eval(obs, task=task, log_detailed=log_detailed)
					# distribution.mean: (T, A) warm-start for next call
					self.planner.update_eval_mean(plan_result.distribution.mean)
				sample_mode = 'argmax' if self.cfg.best_eval and eval_mode else 'gumbel'
				actions, action_std, sample_details, plan_info = self.planner.sample_from_distribution(
					plan_result,
					mode=sample_mode,
				)
				plan_info['std'] = Planner._detach_nested(action_std)
				plan_info['mean'] = Planner._detach_nested(plan_result.distribution.mean)
				selected_index = int(sample_details['index'])  # chosen candidate idx
				extras = plan_result.distribution.extras  # contains per-parent metrics tensors
				if self.cfg.enable_train_planner and not eval_mode:
					plan_info['chosen_parent_index'] = selected_index
					weights = extras.get('weights')  # (parents,)
					if weights is not None:
						plan_info['chosen_parent_weight'] = float(weights[selected_index].item())
					value_max = extras.get('value_max')  # (parents,)
					if value_max is not None:
						plan_info['chosen_value_max'] = float(value_max[selected_index].item())
					disagreement = extras.get('disagreement')  # (parents,)
					if disagreement is not None:
						plan_info['chosen_disagreement'] = float(disagreement[selected_index].item())
					value_head = extras.get('value_head')  # (parents,)
					if value_head is not None:
						plan_info['chosen_value_max_head'] = int(value_head[selected_index].item())
					per_step = extras.get('per_step_disagreement')  # (T, parents)
					if per_step is not None:
						plan_info['chosen_parent_per_step_disagreement'] = Planner._detach_nested(per_step[:, selected_index])
					plan_info['chosen_parent_action'] = Planner._detach_nested(actions)  # (T, parents, A)
					plan_info['chosen_parent_std'] = Planner._detach_nested(action_std)  # (T, parents, A)
				else:
					plan_info['chosen_elite_index'] = selected_index
					plan_info['chosen_std'] = Planner._detach_nested(action_std)  # (T, elites, A)
				first_action = actions[0]  # (A,)
				if eval_mode:
					return first_action, plan_info
				action_std_step = action_std[0] if action_std.dim() > 1 else action_std  # (A,)
				random_noise = torch.randn_like(action_std_step)  # (A,)
				noisy_action = first_action + self.cfg.train_act_std_coeff * action_std_step * random_noise
				return noisy_action.clamp(-1, 1), plan_info
			z = self.model.encode(obs, task)  # z: (1, L)
			action, info = self.model.pi(z, task, use_ema=self.cfg.policy_ema_enabled)
			if eval_mode:
				action = info["mean"]  # mean: (1, A)
			return action[0], {}
